	...
	public function getMenuHtml($groupId) {
		...
		if ($menuCollection != NULL) {
			...
			if (str_replace(' ','',$groupInfo['responsive']) == "menu-creator-pro-rp-switcherside-panel") {
					$DHTML .= '<ul class="responsive-menu-button menu-creator-pro nav navbar-nav"><li class="switcher"><a id="responsive-menu-button-'.$groupId.'" href="#menu-group-html-'.$groupId.'"><i class="mcp-icon fa-bars"></i>Menu</a></li></ul>';
			}
			$DHTML .= "<ul class='level0 " . $this->getGroupOptions($groupId) . " nav navbar-nav'>";
					...
					if ($data ['block'] != '') {
						...
						if (count ( $data ['childcollection'] ) > 0) {
							...
							if ($data['isAutoShowSub'] == true) {
								...
								$DHTML .= "<div class='grid-container2'>";
								$DHTML .= $this->autoSub ( $menu->getUrlValue (), $menu->getAutosub (), false, 2, $arrowForSub, $arrowForSub );
								$DHTML .= "</div>";
							}
						}   
					}
							...
							if ($menu1->getType () != 6) {
								...
								if (count ( $normalArray ) > 0) {
										...
										if (in_array ( $menu2->getMenuId (), $normalArray )) {
											if ($menu2->getType () == 8) {
												...
												if (count ( $data1 ['childcollection'] ) > 0) {
														...
														if ($menu3->getType () == 8) {
															...
															if (count ( $data2 ['childcollection'] ) > 0) {
																	...
																	if ($menu4->getType () == 8) {
																		...
																		if (count ( $data3 ['childcollection'] ) > 0) {
																			...
																			if ($data3['isAutoShowSub']) {
																				$DHTML .= "<div class='grid-container2'>";
																				$DHTML .= $this->autoSub ( $menu4->getUrlValue (), $menu4->getAutosub (), false, 2, $arrowForSub );
																				$DHTML .= "</div>";
																			}
																		}
																		$DHTML .= '</li>';
																...
																if ($data2['isAutoShowSub']) {
																	$DHTML .= "<div class='grid-container2'>";
																	$DHTML .= $this->autoSub ( $menu3->getUrlValue (), $menu3->getAutosub (), false, 2, $arrowForSub );
																	$DHTML .= "</div>";
																}
															}
															$DHTML .= "</li>";
													...
													if ($data1 ['isAutoShowSub'] == true) {
														$DHTML .= "<div class='grid-container2'>";
														$DHTML .= $this->autoSub ( $menu2->getUrlValue (), $menu2->getAutosub (), false, 2, $arrowForSub );
														$DHTML .= "</div>";
													}
												}
												$DHTML .= "</li>";
									...
									if ($headerData['isAutoShowSub']) {
										...
										$DHTML .= "<div class='grid-container2'>";
										$DHTML .= $this->autoSub ( $menu1->getUrlValue (), $menu1->getAutosub (), $showMenuInLevel2, 2, $arrowForSub );
										$DHTML .= "</div>";
									}
								}
							} else {
