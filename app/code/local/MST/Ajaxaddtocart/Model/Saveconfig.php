<?php
class MST_Ajaxaddtocart_Model_Saveconfig extends Mage_Core_Model_Config_Data {
    ...
    protected function _afterSave() {
		...
		*/
		...
		$get_content_id = Mage::helper('ajaxaddtocart')->get_div($file,"valid_licence");
		...
		if(!empty($get_content_id[0])) {
			...
			if ( count($rakes) > 0 ) {
				foreach ( $rakes as $rake )  {
					$update = Mage::getModel('ajaxaddtocart/license')->load( $rake->getLicenseId() );
					$update->setPath($path);
				...
				$new = Mage::getModel('ajaxaddtocart/license');
				$new->setPath($path);
